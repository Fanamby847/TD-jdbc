...
public class DataRetriever {
    ...
    Order saveOrder(Order orderToSave) {
        ...
        if (orderToSave.getOrderType() == null) {
            orderToSave.setOrderType(OrderType.EAT_IN);
        }
        ...
        try( Connection connection = dbConnection.getConnection()) {
            ...
            preparedStatement.setString(1, orderToSave.getOrderType().name());
            preparedStatement.setString(2, orderToSave.getOrderStatus().name());
            preparedStatement.setInt(3 , orderToSave.getId());
    ...
    Order findOrderByReference(String reference) {
        ...
        try (Connection connection = dbConnection.getConnection()) {
            PreparedStatement preparedStatement = connection.prepareStatement("""
                    select id , reference , creation_datetime , type , status from orders where reference = ?""" );
            ...
            if (resultSet.next()) {
                ...
                order.setCreationDatetime(Instant.from(resultSet.getTimestamp("creation_datetime").toLocalDateTime()));
    ...
    private void attachIngredients(Connection conn, List<DishIngredient> ingredients)
        ...
        String attachSql = """
                    insert into dish_ingredient (id, ingredient_id, dish_id, required_quantity, unit)
                    values (?, ?, ?, ?, ?::Unit)
                """;
        ...
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(
                    """
                            ...
                            from ingredient join dish_ingredient di on di.ingredient_id = ingredient.id where id_dish = ?;
                            """);
